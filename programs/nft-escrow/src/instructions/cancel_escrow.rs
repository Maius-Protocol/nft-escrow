...
pub fn handler(ctx: Context<CancelEscrow>) -> Result<()> {
    ...
    if signer.key() != escrow.bob_key && signer.key() != escrow.alice_key {
        return Err(ErrorCodes::InvalidBobOrAlice.into());
    }
    ...
    if signer.key() == escrow.alice_key {
        escrow.status = StatusStage::CancelByAlice;
    }

    if signer.key() == escrow.bob_key {
        escrow.status = StatusStage::CancelByBob;
    }

    let vault_token_amount = &ctx.accounts.vault_token.amount;
    if *vault_token_amount > 0 as u64 {
        transfer(CpiContext::new_with_signer(
            ctx.accounts.token_program.to_account_info(),
            Transfer {
                from: ctx.accounts.vault_token.to_account_info(),
                to: ctx.accounts.bob_taker_token_account.to_account_info(),
    ...
    transfer(CpiContext::new_with_signer(
        ...
        Transfer {
            ...
            to: ctx.accounts.alice_nft_account.to_account_info(),
            authority: ctx.accounts.authority_vault.to_account_info(),
        },
        &[&["authority".as_bytes(),
            ctx.accounts.escrow.key().as_ref(),
